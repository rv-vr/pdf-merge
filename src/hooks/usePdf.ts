...
function pdfReducer(state: PdfState, action: PdfAction): PdfState {
  switch (action.type) {
    ...
    case 'doc_reset':
      return { ...state, pdfFile: null, pdfBytes: null, totalPages: 0, currentPage: 1 };
    default:
      return state;
  }
}
...
export function usePdf(_view: 'upload' | 'editor') {
  ...
  return {
    ...
    setTotalPages: (total: number) => dispatch({ type: 'total_pages_set', totalPages: total }),
    setPdfDimensions: (dimensions: { width: number; height: number }) => dispatch({ type: 'dimensions_set', dimensions }),
