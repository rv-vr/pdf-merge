...
export function Ruler({ orientation, containerWidth, containerHeight, zoom, onGuideCreate, onGuidePreview }: RulerProps) {
  const SIZE = 18;
  ...
  const ticks = useMemo(() => {
    ...
    for (let i = 0; i <= 100; i += tickInterval) {
      ...
      result.push({ pos: px, label: String(i), isMajor: i % (tickInterval * 2) === 0 || i === 0 || i === 100 });
    }
    return result;
  }, [length, tickInterval]);

  const getPctFromEvent = useCallback((ev: MouseEvent): { pct: number; onCanvas: boolean } | null => {
          ...
          <g key={tick.pos}>
            {isHorizontal ? (
              <>
                <line
                  x1={tick.pos} y1={0}
                  x2={tick.pos} y2={tick.isMajor ? SIZE - 2 : SIZE - 8}
                  stroke="currentColor"
                  strokeWidth={0.5}
                />
                {tick.isMajor && (
                  <text
                    x={tick.pos + 2} y={SIZE - 4}
                    fill="currentColor"
                    fontSize={8}
              ...
              <>
                <line
                  x1={0} y1={tick.pos}
                  x2={tick.isMajor ? SIZE - 2 : SIZE - 8} y2={tick.pos}
                  stroke="currentColor"
                  strokeWidth={0.5}
                />
                {tick.isMajor && (
                  <text
                    x={SIZE - 4} y={tick.pos - 2}
                    fill="currentColor"
                    fontSize={8}
                    fontFamily="monospace"
                    textAnchor="end"
