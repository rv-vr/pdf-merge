...
function getCanvasRect(): DOMRect | null {
  return document.querySelector('[data-canvas-container]')?.getBoundingClientRect() ?? null;
}

function toVp(pct: number, isVert: boolean): number {
  const r = getCanvasRect();
  if (!r) return 0;
  return isVert ? r.left + (pct / 100) * r.width : r.top + (pct / 100) * r.height;
}
...
export function GuidesOverlay({
  ...
  const dragRef = useRef<{ id: string; orientation: 'horizontal' | 'vertical' } | null>(null);
  const [_, forceUpdate] = useState(0);
  const [wsBounds, setWsBounds] = useState({ top: 0, height: 0, left: 0, width: 0 });
  ...
  // Throttled scroll handler — at most once per frame
  useEffect(() => {
    const ws = document.querySelector('[data-workspace]');
    if (!ws) return;
    let ticking = false;
    ...
    const onResize = () => { refreshWsBounds(); forceUpdate((n) => n + 1); };
  ...
  useEffect(() => { forceUpdate((n) => n + 1); refreshWsBounds(); }, [versionKey, refreshWsBounds]);

  // Cull guides outside viewport to avoid rendering off-screen
  const isInViewport = useCallback((guide: Guide): boolean => {
    const isVert = guide.orientation === 'vertical';
    const vp = toVp(guide.position, isVert);
    if (wsBounds.height <= 0) return false;
    if (isVert) return vp >= wsBounds.left && vp <= wsBounds.left + wsBounds.width;
    return vp >= wsBounds.top && vp <= wsBounds.top + wsBounds.height;
  }, [wsBounds]);

  const pageGuides = guides.filter((g) => g.page === currentPage && isInViewport(g));

  const handleMouseDown = useCallback((e: React.MouseEvent, guide: Guide) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    const handleMouseMove = (ev: MouseEvent) => {
      ...
      const r = getCanvasRect();
      if (!r) return;
      const pct = dragRef.current.orientation === 'vertical'
  ...
  if (pageGuides.length === 0 && (ghostPosition == null || ghostPosition < 0)) return null;
    ...
    >
      {pageGuides.map((guide) => {
        ...
        return (
          ...
          >
            <div className={`absolute ${isVert ? 'inset-y-0 -translate-x-1/2 w-4 cursor-col-resize' : 'inset-x-0 -translate-y-1/2 h-4 cursor-row-resize'}`} />
            <div className={`${isVert ? 'h-full w-px' : 'w-full h-px'} bg-blue-500/60`} />
          </div>
        );
      })}
      {ghostPosition != null && ghostPosition >= 0 && (
        <div
          ...
          style={{
            position: 'absolute',
            [ghostOrientation === 'vertical' ? 'left' : 'top']: toVp(ghostPosition, ghostOrientation === 'vertical'),
            [ghostOrientation === 'vertical' ? 'top' : 'left']: 0,
            [ghostOrientation === 'vertical' ? 'width' : 'height']: 1,
            [ghostOrientation === 'vertical' ? 'bottom' : 'right']: 0,
          }}
        >
          <div className={`${ghostOrientation === 'vertical' ? 'h-full w-px' : 'w-full h-px'} bg-blue-500/80`} />
        </div>
      )}
    </div>
  );
